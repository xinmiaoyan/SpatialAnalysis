...
for(dim in seq(20,50,10)){
    for(res in seq(0.3,1,0.2)){
        dir = paste('dim_',dim,'_res_',res,sep='')
        dir.create(dir)
        setwd(dir)
        ...
        ggsave(plot = p, file = paste('03_clustering_',dim,'_',res,'.pdf',sep=''))
        ...
        write.csv(sp_markers, '04_sp_markers.csv')
